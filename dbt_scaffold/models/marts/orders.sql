...
joined as (
    ...
    select
        ...
        case
            when coalesce(order_items_summary.count_food_items, 0) > 0 then true
            else false
        end as is_food_order,
        case
            when coalesce(order_items_summary.count_drink_items, 0) > 0 then true
            else false
        end as is_drink_order
    from orders
    left join order_items_summary
        on order_items_summary.order_id = orders.order_id
