...
CREATE OR REPLACE TABLE JAFFLE_SHOP.LEGACY_PUBLIC.ORDER_ITEMS_ENRICHED AS
WITH
SUPPLY_COST_PER_PRODUCT AS (
    SELECT
        PRODUCT_ID,
        SUM(SUPPLY_COST) AS SUPPLY_COST
    FROM JAFFLE_SHOP.LEGACY_PUBLIC.STG_SUPPLIES
    GROUP BY PRODUCT_ID
)
SELECT
    OI.ORDER_ITEM_ID,
    ...
    P.IS_DRINK_ITEM,
    SC.SUPPLY_COST
FROM JAFFLE_SHOP.LEGACY_PUBLIC.STG_ORDER_ITEMS OI
JOIN JAFFLE_SHOP.LEGACY_PUBLIC.STG_ORDERS    O  ON O.ORDER_ID    = OI.ORDER_ID
JOIN JAFFLE_SHOP.LEGACY_PUBLIC.STG_PRODUCTS  P  ON P.PRODUCT_ID  = OI.PRODUCT_ID
